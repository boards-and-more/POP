...
DEFINE_RW_PROPERTY(POPDecayAnimationState, deceleration, setDeceleration:, CGFloat, __state->toVec = (VectorRef)NULL;);
  ...
  if (valueType != kPOPValueUnknown) {
    ...
    if (!vec_equal(vec, __state->velocityVec)) {
      ...
      if (__state->active && __state->paused) {
        __state->fromVec = (VectorRef)NULL;
        __state->setPaused(false);
      }
    }
  } else {
    __state->velocityVec = (VectorRef)NULL;
